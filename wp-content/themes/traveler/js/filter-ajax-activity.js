(function ($) {
    ...
    $(document).on('click', '.checkbox-filter-ajax, a.pagination', function (e) {
        if ($('#ajax-filter-content').length > 0) {
            ...
            var start = $('form[post_type="st_activity"] .irs-from').text();
            var end = $('form[post_type="st_activity"] .irs-to').text();
            ...
            var tax_string = active_tax.toString();
            ...
            if(!$('#jactivity-orderby').length || checkOrderBy){
            ...
            data['page'] = pageNum;
            data['isajax'] = '1';
            data['jcategory'] = $('#jactivity-category').val();

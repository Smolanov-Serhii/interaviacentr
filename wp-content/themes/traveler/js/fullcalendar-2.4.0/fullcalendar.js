		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							if (overrideText) {
								...
								innerHtml = "<span class=' fc-icon-" + normalIcon + "'></span>";
							}
							else {
				...
				if (groupChildren.length >= 1) {
					...
					if(position == 'left'){
						var html_button_change_month = '<label for="label_calendar_change_month" class="label_calendar_change_month">Change month</label><select id="label_calendar_change_month" class="calendar_change_month '+ tm + '-corner-left' +'">';
						for(var i = 1; i <= 12; i++){
							html_button_change_month += '<option value="' + i +'">' + i +'</option>';
						}
						html_button_change_month += '</select>';
						button_change_month = $(html_button_change_month);
						button_change_month.on('change', function(event){
							$('body').trigger('calendar.change_month', [$(this).val()]);
						});
						groupEl.append(button_change_month);
					}
					//---- .End

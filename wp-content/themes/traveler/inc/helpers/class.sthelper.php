...
if (!function_exists('st_paging_nav')) {
    ...
    function st_paging_nav($title = null, $query = false, $num_pages = false) {
        ...
        $links = paginate_links(array(
            ...
            'prev_text' => __('&larr; Previous', 'traveler'),
            'next_text' => __('Next &rarr;', 'traveler'),
        ));
        if ($links) :
            ?>
    ...
    function st_get_avatar_in_list_service($post_id, $size = 35, $return_link = false) {
        ...
        if (st()->get_option('avatar_in_list_service', 'off') == 'on') {
            ...
            if ($partner_page != '') {
                ...
                $author_link = esc_url(add_query_arg(array('partner_id' => $post_author_id), $partner_link));
            } else {
            ...
            if ($return_link) {
                ...
                return '<a href="' . $user_link . '" target="_blank">' . st_get_profile_avatar($post_author_id, $size) . '</a>';
            }
            return '<a href="' . $author_link . '" target="_blank" title="' . get_the_author() . '">' . st_get_profile_avatar($post_author_id, $size) . '</a>';

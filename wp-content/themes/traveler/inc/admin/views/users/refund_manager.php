...
echo '<h2>' . __('Refund Manager', 'traveler') . '</h2>';
?>
<?php
...
<?php if (have_posts()): ?>
        ...
        while (have_posts()): the_post();
            ...
            if ($post->st_booking_post_type == 'st_tours') {
                ...
                if ($type_tour == 'daily_tour') {
                    ...
                    if ($date) {
                        $date = __("Check in : ", 'traveler') . date_i18n($format, $post->check_in_timestamp) . "<br>";
            ...
            $action_cancel = '';
            if ((float)$post->cancel_refund > 0 && $post->cancel_refund_status == 'pending') {
                $action_cancel = '<br /><a data-toggle="modal" data-target="#with-refund-modal" href="javascript: void(0);" class="with_a_refund btn btn-primary btn-xs" data-order_id="' . esc_attr($post->order_item_id) . '" data-order_encrypt="' . esc_attr(TravelHelper::st_encrypt($post->order_item_id)) . '">' . __('with refund', 'traveler') . '</a>';
            }
            $action = '';
            ...
            ?>
            <tr>
                <td class="booking-history-type <?php echo esc_attr($post->st_booking_post_type); ?>">
                    <?php echo balanceTags($class_user->get_icon_type_order_item($post->st_booking_id)); ?>
                </td>
                <td>
                    <a href="<?php echo esc_url($class_user->get_link_order_item($post->st_booking_id)); ?>"><?php echo esc_html($class_user->get_title_order_item($post->st_booking_id)) ?></a>
                </td>
                <td class="hidden-xs"><?php echo esc_attr($address); ?></td>
                <td class="hidden-xs">
                    <?php echo date_i18n($format, strtotime($post->created)); ?>
                </td>
                <td class="hidden-xs"><?php echo wp_kses($date, array('i' => ['class' => []])); ?></td>
                <td><?php echo TravelHelper::format_money_raw($price, $currency); ?></td>
                <td class="hidden-xs"><?php echo esc_html($status_string) . $action_cancel; ?></td>
            </tr>
        <?php endwhile; ?>
    </table>
    ...
    echo '<h3>' . __('The refund list is empty', 'traveler') . '</h3>';
endif;
?>
    ...
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="withRefundLabel"><?php echo __('Refund Infomation', 'traveler'); ?></h4>
            </div>
            ...
            <div class="modal-footer">
                <button id="" type="button"
                        class="refund_complete next btn btn-primary hidden"><?php echo __('Complete this Refund', 'traveler'); ?></button>
                <button type="button" class="btn btn-default close-modal" data-dismiss="modal"><?php echo __("Close", 'traveler'); ?></button>
            </div>
        </div>
    </div>
</div>

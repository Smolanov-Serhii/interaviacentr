...
<div class="form-group form-group-<?php echo esc_attr($field_size)?> form-group-icon-left flight-input">
    ...
    <div class="st-select-wrapper">
        <input  autocomplete="off" type="text" name="destination_name" value="<?php echo esc_attr($destination_name); ?>" class="form-control st-flight-location-name destination <?php echo esc_attr($is_required); ?>" placeholder="<?php if($placeholder) echo esc_attr($placeholder); ?>">
        <select  name="destination" class="st-location-id st-hidden" placeholder="<?php if($placeholder) echo esc_attr($placeholder); ?>" tabindex="-1">
            <option value=""></option>
            <?php

...
<div class="form-group form-group-<?php echo esc_attr($field_size)?> form-group-icon-left flight-input">
    ...
    <div class="st-select-wrapper">
        <input  autocomplete="off" type="text" name="origin_name" value="<?php echo esc_attr($origin_name); ?>" class="form-control st-flight-location-name <?php echo esc_attr($is_required); ?>" placeholder="<?php if($placeholder) echo esc_attr($placeholder); ?>">
        <select  name="origin" class="st-flight-location-id st-hidden" placeholder="<?php if($placeholder) echo esc_attr($placeholder); ?>" tabindex="-1">
            <option value=""></option>
            <?php
            if(!empty($list_airport[0]->name)){
                foreach($list_airport as $key => $val){
                    ...
                    if(!empty($iata_airport)){
                        echo '<option '.selected($origin, $iata_and_key, false).' value="'.esc_attr($iata_and_key).'">'.st_flight_get_airport_text($val->term_id, $val->name).'</option>';
                    }
                }
            }
            ?>

...
if(!empty($list_style_2)){
    ...
    ?>
    <div class="st-special-services style-4">
        <?php
        foreach($data as $key => $val){
        ?>
        <div class="item-service <?php echo (($key+1)%2 != 0)?'even-item-2':''; ?>">
            <?php
            $bg = '';
            if(!empty($val['image'])){
                $img = wp_get_attachment_image_src($val['image'], array(480, 600), false);
                if(!empty($img[0])){

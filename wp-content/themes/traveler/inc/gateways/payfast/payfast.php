        ...
        class ST_Payfast_Gateway extends STAbstactPaymentGateway
        {
					...
					if(isset(STTravelCode::$plugins_data['Version']))
					{
						...
						if(version_compare('1.3.2',$version,'>'))
						{
							...
							<div class="error settings-error notice is-dismissible">
								...
								<p>
									<?php printf(__('<strong>PayFast</strong> require %s version %s or above. Your current is %s','traveler'),'<strong><em>'.__('Traveler Code','traveler').'</em></strong>','<strong>1.3.2</strong>','<strong>'.$version.'</strong>'); ?>
								</p>
								<p>
									<a href="http://shinetheme.com/demosd/documentation/how-to-update-the-theme-2/" target="_blank"><?php _e('Learn how to update it','traveler')?></a>
									|
									<a href="<?php echo admin_url('index.php?st_dismiss_payfast_notice=1') ?>" class="dismiss-notice" target="_parent"><?php _e('Dismiss this notice','traveler')?></a>
								</p>
								<button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php _e('Dismiss this notice','traveler')?>.</span></button>
							</div>
							<?php
						}
					}
            ...
            function do_checkout($order_id)
            {
                ...
                if(!$pp_link)
                {
                    return array(
                        'status'=>false,
                        'message'=>__('Can not get PayFast Authorize URL.','traveler')
                    );
                }else{
            ...
            function get_option_fields()
            {
                return array(
                    array(
                        ...
                        'label'         =>__('Merchant Id','traveler'),
                        'type'          =>'text',
                        'section'       =>'option_pmgateway',
                        'desc'          =>__('Your Merchant Id','traveler'),
                        'condition'     =>'pm_gway_st_payfast_enable:is(on)'
                    ),
                    array(
                        'id'            =>'pay_fast_merchant_key',
                        'label'         =>__('Merchant Key','traveler'),
                        'type'          =>'text',
                        'section'       =>'option_pmgateway',
                        'desc'          =>__('Your Merchant Key','traveler'),
                        'condition'     =>'pm_gway_st_payfast_enable:is(on)'
                    ),
                    array(
                        'id'            =>'pay_fast_enable_sandbox',
                        'label'         =>__('Enable Sandbox','traveler'),
                        'type'          =>'on-off',
                        'section'       =>'option_pmgateway',
                        'std'           =>'on',

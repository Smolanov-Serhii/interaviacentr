    ...
    class STGatewayPaypal_AdaptivePayment extends STAbstactPaymentGateway
    {
        ...
        function _add_info_after_name_payment_method( $order_id )
        {
            ...
            if($payment_method == $this->_gateway_id and !empty( $payment_mode )) {
                echo "<br>";
                echo "<b>" . esc_html__( "Type: " , 'traveler' ) . "</b>";
                if("parallel" == $payment_mode) {
                    echo esc_html__( 'Parallel' , 'traveler' );
                } elseif('chained' == $payment_mode) {
                    echo esc_html__( 'Chained' , 'traveler' );
                } else {
                    echo esc_html__( 'Delayed Chained' , 'traveler' );
                }
                ...
                if("delayed_chained" == $payment_mode) {
                    ...
                    if($st_is_split_payment_receive == "yes") {
                        $status = '<span class="status_split_complete">' . esc_html__( "Complete" , 'traveler' ) . "</span>";
                    } else {
                        $status = '<span class="status_split_pending">' . esc_html__( "AwaitingPay" , 'traveler' ) . "</span>";
                    }
                    echo "<b>" . esc_html__( "Status: " , 'traveler' ) . "</b>" . $status;
                }
                if("delayed_chained" == $payment_mode and $st_is_split_payment_receive != "yes") {
                    echo "<br>";
                    $url = add_query_arg( 'st_execute_delay_payment' , $order_id );
                    echo '<a href="' . esc_url( $url ) . '">' . esc_html__( "Pay Now" , 'traveler' ) . '</a>';
                }
            }
        }
        ...
        function check_complete_purchase( $order_id )
        {
            if($order_id and false !== get_post_status( $order_id )) {
                ...
                if($payment_status == 'COMPLETED' || $payment_status == 'PROCESSING' || $payment_status == 'INCOMPLETE') {
                    ...
                    return [ 'status'  => false ,
                             'message' => __( 'Acknowledgement Received Payment Failed' , 'traveler' )
                    ];
                }
            } else {
        ...
        function do_checkout( $order_id )
        {
            ...
            if($pay_result) {
                ...
                return array(
                    'status'  => false ,
                    'message' => __( 'Sorry, Something went wrong' , 'traveler' ) ,
                    'data'    => '' ,
                );
            }
        }
        ...
        function get_name()
        {
            return __( 'Paypal Adaptive' , 'traveler' );
        }
        ...
        function get_option_fields()
        {
            return array(
                array(
                    ...
                    'label'   => __( 'Paypal Email' , 'traveler' ) ,
                    'type'    => 'text' ,
                    'section' => 'option_pmgateway' ,
                    'desc'    => __( 'Your Payal Email Account' , 'traveler' ),
                    'condition' => 'pm_gway_' . $this->_gateway_id . '_enable:is(on)'
                ) ,
                array(
                    'id'      => 'paypal_adaptivepayment_payment_mode' ,
                    'label'   => __( "Payment Mode" , 'traveler' ) ,
                    'type'    => 'select' ,
                    'choices' => array(
                        array(
                            'value' => 'parallel' ,
                            'label' => __( 'Parallel' , 'traveler' )

                        ) ,
                        array(
                            'value' => 'chained' ,
                            'label' => __( 'Chained' , 'traveler' )
                        ) ,
                        array(
                            'value' => 'delayed_chained' ,
                            'label' => __( 'Delayed Chained' , 'traveler' )
                        ) ,
                    ) ,
                    'section' => 'option_pmgateway' ,
                    'condition' => 'pm_gway_' . $this->_gateway_id . '_enable:is(on)'
                ...
                array(
                    ...
                    'label'   => __( "Payment Parallel Fees by" , 'traveler' ) ,
                    'type'    => 'select' ,
                    'choices' => array(
                        array(
                            'value' => 'SENDER' ,
                            'label' => __( 'Sender' , 'traveler' )
                        ) ,
                        array(
                            'value' => 'EACHRECEIVER' ,
                            'label' => __( 'Each Receiver' , 'traveler' )
                        ) ,
                    ) ,
                    'section' => 'option_pmgateway' ,
                ...
                array(
                    ...
                    'label'   => __( "Payment Chained Fees by" , 'traveler' ) ,
                    'type'    => 'select' ,
                    'choices' => array(
                        array(
                            'value' => 'PRIMARYRECEIVER' ,
                            'label' => __( 'Primary Receiver' , 'traveler' )
                        ) ,
                        array(
                            'value' => 'EACHRECEIVER' ,
                            'label' => __( 'Each Receiver' , 'traveler' )
                ...
                array(
                    ...
                    'label'        => __( 'No. of Days to Execute Payment to Receiver' , 'traveler' ) ,
                    'std'          => '90' ,
                    'type'         => 'text' ,
                    'section'      => 'option_pmgateway' ,
                    'min' => '1',
                ...
                array(
                    ...
                    'label'   => __( 'Paypal Enable Sandbox' , 'traveler' ) ,
                    'type'    => 'on-off' ,
                    'section' => 'option_pmgateway' ,
                    'std'     => 'on' ,
                    'desc'    => __( 'Allow you to enable sandbox mod for testing' , 'traveler' ),
                    'condition' => 'pm_gway_' . $this->_gateway_id . '_enable:is(on)'
                ) ,
                array(
                    'id'      => 'paypal_adaptivepayment_api_username' ,
                    'label'   => __( 'Paypal API Username' , 'traveler' ) ,
                    'type'    => 'text' ,
                    'section' => 'option_pmgateway' ,
                    'desc'    => __( 'Your Paypal API Username' , 'traveler' ),
                    'condition' => 'pm_gway_' . $this->_gateway_id . '_enable:is(on)'
                ) ,
                array(
                    'id'      => 'paypal_adaptivepayment_api_password' ,
                    'label'   => __( 'Paypal API Password' , 'traveler' ) ,
                    'type'    => 'text' ,
                    'section' => 'option_pmgateway' ,
                    'desc'    => __( 'Your Paypal API Password' , 'traveler' ),
                    'condition' => 'pm_gway_' . $this->_gateway_id . '_enable:is(on)'
                ) ,
                array(
                    'id'      => 'paypal_adaptivepayment_api_signature' ,
                    'label'   => __( 'Paypal API Signature' , 'traveler' ) ,
                    'type'    => 'text' ,
                    'section' => 'option_pmgateway' ,
                    'desc'    => __( 'Your Paypal API Signature' , 'traveler' ),
                    'condition' => 'pm_gway_' . $this->_gateway_id . '_enable:is(on)'
                ) ,
                array(
                    'id'      => 'paypal_adaptivepayment_application_id' ,
                    'label'   => __( 'Application ID' , 'traveler' ) ,
                    'type'    => 'text' ,
                    'section' => 'option_pmgateway' ,
                    'desc'    => __( 'Your Paypal API Signature' , 'traveler' ),
                    'condition' => 'pm_gway_' . $this->_gateway_id . '_enable:is(on)'
        ...
        function _handling_refund(){
            ...
            if($is_checked and !empty($cancel_data) and $st_payment_refund != 'yes'){
                ...
                if($response->responseEnvelope->ack == "Success"){
                    ...
                    $res['status'] = 'true';
                    $res['message'] = esc_html__("Refund success",'traveler');
            ...
            if($st_payment_refund == "yes"){
                $res['status'] = 'true';
                $res['message'] = esc_html__("You paid for the buyer",'traveler');
                echo json_encode($res);
                wp_die();
            }
        }
        ...
        function _execute_delay_payment_on_scheduled( $order_id = false )
        {
            if(!empty( $order_id )) {
                ...
                if($pay_respon->responseEnvelope->ack == "Failure") {
                    ...
                    $message                                            = sprintf( esc_html__( "This order #%s cannot execute payment for partner" , 'traveler' ) , $order_id );
                    $message .= "<br>" . esc_html__( "Message: " , 'traveler' ) . $pay_respon->error[ 0 ]->message;
                    $st_paypal_adaptivepayment_admin_notice[ 'message' ] = $message;
                } else {
                    global $st_paypal_adaptivepayment_admin_notice;
                    $st_paypal_adaptivepayment_admin_notice[ 'status' ]  = 'success ';
                    $message                                             = sprintf( esc_html__( "This order #%s is executed payment for partner" , 'traveler' ) , $order_id );
                    $st_paypal_adaptivepayment_admin_notice[ 'message' ] = $message;
                }

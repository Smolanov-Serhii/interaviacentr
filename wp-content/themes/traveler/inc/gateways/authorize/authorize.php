...
if ( ! class_exists( 'ST_Authorize_Payment_Gateway' ) ) {
	class ST_Authorize_Payment_Gateway extends STAbstactPaymentGateway {
				...
				if ( isset( STTravelCode::$plugins_data['Version'] ) ) {
					...
					if ( version_compare( '1.3.2', $version, '>' ) ) {
						...
						<div class="error settings-error notice is-dismissible">
							...
							<p>
								<?php printf( __( '<strong>Authorize.net</strong> require %s version %s or above. Your current is %s', 'traveler' ), '<strong><em>' . __( 'Traveler Code', 'traveler' ) . '</em></strong>', '<strong>1.3.2</strong>', '<strong>' . $version . '</strong>' ); ?>
							</p>
							<p>
		...
		function get_option_fields() {
			return array(
				array(
					...
					'label'     => __( 'Login ID', 'traveler' ),
					'type'      => 'text',
					'section'   => 'option_pmgateway',
					'desc'      => __( 'Login ID', 'traveler' ),
					'condition' => 'pm_gway_st_authorize_enable:is(on)'
				),
				array(
					'id'        => 'authorize_transaction_key',
					'label'     => __( 'Transaction Key', 'traveler' ),
					'type'      => 'text',
					'section'   => 'option_pmgateway',
					'desc'      => __( 'Transaction Key', 'traveler' ),
					'condition' => 'pm_gway_st_authorize_enable:is(on)'
				),
				array(
		...
		function _pre_checkout_validate() {
			...
			$validate->set_rules('st_authorize_card_name',__("Card Name",'traveler'),'required');
			$validate->set_rules('st_authorize_card_number',__("Card Number",'traveler'),'required');
			$validate->set_rules('st_authorize_card_expiry_month',__("Expiry Month",'traveler'),'required');
			$validate->set_rules('st_authorize_card_expiry_year',__("Expiry Year",'traveler'),'required');
			$validate->set_rules('st_authorize_card_code',__("Card Code",'traveler'),'required');
		...
		function get_name() {
			return __( 'Authorize.net', 'traveler' );
		}

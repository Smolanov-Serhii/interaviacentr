...
if ( ! class_exists( 'ST_TwoCheckout_Payment_Gateway' ) ) {
	class ST_TwoCheckout_Payment_Gateway extends STAbstactPaymentGateway {
				...
				if ( isset( STTravelCode::$plugins_data['Version'] ) ) {
					...
					if ( version_compare( '1.3.2', $version, '>' ) ) {
						...
						<div class="error settings-error notice is-dismissible">
							...
							<p>
								<?php printf( __( '<strong>TwoCheckout</strong> require %s version %s or above. Your current is %s', 'traveler' ), '<strong><em>' . __( 'Traveler Code', 'traveler' ) . '</em></strong>', '<strong>1.3.2</strong>', '<strong>' . $version . '</strong>' ); ?>
							</p>
							<p>
		...
		function get_option_fields() {
			return array(
				array(
					...
					'label'     => __( 'Account Number', 'traveler' ),
					'type'      => 'text',
					'section'   => 'option_pmgateway',
					'desc'      => __( 'Account Number', 'traveler' ),
					'condition' => 'pm_gway_st_twocheckout_enable:is(on)'
				),
				array(
					'id'        => 'twocheckout_public_key',
					'label'     => __( 'Public Key', 'traveler' ),
					'type'      => 'text',
					'section'   => 'option_pmgateway',
					'desc'      => __( 'Public Key', 'traveler' ),
					'condition' => 'pm_gway_st_twocheckout_enable:is(on)'
				),
				array(
					'id'        => 'twocheckout_private_key',
					'label'     => __( 'Private Key', 'traveler' ),
					'type'      => 'text',
					'section'   => 'option_pmgateway',
					'desc'      => __( 'Private Key', 'traveler' ),
					'condition' => 'pm_gway_st_twocheckout_enable:is(on)'
				),
				array(
		...
		function _pre_checkout_validate() {
			...
			$validate->set_rules('st_twocheckout_card_name',__("Card Name",'traveler'),'required');
			$validate->set_rules('st_twocheckout_card_number',__("Card Number",'traveler'),'required');
			$validate->set_rules('st_twocheckout_card_expiry_month',__("Expiry Month",'traveler'),'required');
			$validate->set_rules('st_twocheckout_card_expiry_year',__("Expiry Year",'traveler'),'required');
			$validate->set_rules('st_twocheckout_card_code',__("Card Code",'traveler'),'required');
		...
		function do_checkout( $order_id ) {
			...
			$purchase = [
				...
				'transactionId' => esc_html__( 'Order:', 'traveler' ) . ' ' . esc_attr( $order_id ) . '-' . esc_attr( $order_id ),
		...
		function get_name() {
			return __( 'TwoCheckout', 'traveler' );
		}

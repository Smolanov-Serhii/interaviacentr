    ...
    if ( !class_exists( 'STRoom' ) ) {
        class STRoom extends TravelerObject
        {
            ...
            function init()
            {
                ...
                add_action('save_post', [$this, '_reSaveHotelParentCalendar'], 10, 2);
            }

            public function _reSaveHotelParentCalendar($post_id, $post_obj){
                global $wpdb;
                $parent_id = get_post_meta($post_id, 'room_parent', true);
                $sql = $wpdb->prepare("SELECT id, post_id, parent_id FROM {$wpdb->prefix}st_room_availability WHERE post_id = %d AND parent_id != %d GROUP BY parent_id", $post_id, $parent_id);

                $res = $wpdb->get_results($sql, ARRAY_A);

                if(!empty($res)){
                    foreach ($res as $k => $v) {
                        $sql_del = $wpdb->prepare("DELETE FROM {$wpdb->prefix}st_room_availability WHERE post_id = %d AND parent_id = %d", $post_id, $v['parent_id']);
                        $res = $wpdb->query($sql_del);
                    }
                }
            }
            ...
            {
                return st()->load_template( 'hotel-room/cart_item_html', NULL, [ 'item_id' => $item_id ] );
            }

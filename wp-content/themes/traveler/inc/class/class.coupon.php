        ...
        class STCoupon extends TravelerObject
        {
            ...
            function _add_metabox()
            {
                $this->metabox[] = [
                    ...
                    'title'    => __( 'Coupon Detail', 'traveler' ),
                    'desc'     => '',
                    ...
                    'fields'   => [
                        ...
                        , [
                            'id'      => 'discount_type',
                            'label'   => __( 'Select discount type', 'traveler' ),
                            'type'    => 'select',
                            'desc'    => __( 'Select Discount Type', 'traveler' ),
                            'choices' => [
                                [
                                    'value' => 'cart_amount',
                                    'label' => __( 'Cart discount', 'traveler' )
                                ],
                                [
                                    'value' => 'cart_percent',
                                    'label' => __( 'Cart % discount', 'traveler' )
                        ...
                        [
                            'id'    => 'amount',
                            'label' => __( 'Input discount value', 'traveler' ),
                            'desc'  => __( 'Discount amount', 'traveler' ),
                            'type'  => 'text',
                        ],
                        [
                            'id'    => 'expiry_date',
                            'label' => __( 'Expired date', 'traveler' ),
                            'desc'  => __( 'Select date to expired this discount', 'traveler' ),
                            'type'  => 'date-picker',
                        ],
                        [
                            'id'    => 'restriction_tab',
                            'label' => __( 'Restriction', 'traveler' ),
                            'type'  => 'tab',
                        ],
                        [
                            'id'    => 'minimum_spend',
                            'label' => __( 'Minimum spend', 'traveler' ),
                            'desc'  => __( 'Minimum spend', 'traveler' ),
                            'type'  => 'text',
                        ],
                        [
                            'id'    => 'maximum_spend',
                            'label' => __( 'Maximum spend', 'traveler' ),
                            'desc'  => __( 'Maximum spend', 'traveler' ),
                            'type'  => 'text',
                        ],
                        [
                            'id'    => 'include_products',
                            'label' => __( 'Select service apply for coupon', 'traveler' ),
                            'desc'  => __( 'Select service use and item apply for coupon', 'traveler' ),
                            'type'  => 'product_select_ajax',
                        ],
                        [
                            'id'    => 'exclude_products',
                            'label' => __( 'Select service not apply for coupon', 'traveler' ),
                            'desc'  => __( 'Select service and item not apply for coupon', 'traveler' ),
                            'type'  => 'product_select_ajax',
                        ],
                        [
                            'id'    => 'limit_per_coupon',
                            'label' => __( 'Limited for each coupon', 'traveler' ),
                            'desc'  => __( 'Input number of times use this coupon code. Set empty for use unlimited', 'traveler' ),
                            'type'  => 'text',
                        ],
                        [
                            'id'    => 'limit_per_user',
                            'label' => __( 'Limited for user', 'traveler' ),
                            'desc'  => __( 'Input number of times use this coupon for each user. Set empty for use unlimited', 'traveler' ),
                            'type'  => 'text',
                        ],
                    ]
                ];
            ...
            static function get_coupon_value( $coupon, $total = false, $cart_items = [], $customer_email = false, $check_in = null )
            {
                ...
                if ( !$coupon_object ) return [
                    'status'  => 0,
                    'message' => __( 'Coupon Not Found', 'traveler' )
                ];
                ...
                if ( $expiry_date ) {
                    ...
                    if ( $datediff <= 0 ) {
                        return [
                            'status'  => 0,
                            'message' => __( 'This coupon is expired', 'traveler' )
                        ];
                    }
                }
                ...
                if ( $minimum_spend ) {
                    if ( $minimum_spend > $total ) {
                        return [
                            'status'  => 0,
                            'message' => sprintf( __( 'This coupon is only applicable for bills spent over %s', 'traveler' ), TravelHelper::format_money( $minimum_spend ) )
                        ];
                    }
                }
                ...
                if ( $maximum_spend ) {
                    if ( $maximum_spend < $total ) {
                        return [
                            'status'  => 0,
                            'message' => sprintf( __( 'This coupon is only applicable for bills spent less than %s', 'traveler' ), TravelHelper::format_money( $maximum_spend ) )
                        ];
                    }
                }
                ...
                if ( $required_porduct ) {
                    ...
                    if ( empty( $check ) ) {
                        return [
                            'status'  => 0,
                            'message' => sprintf( __( 'This coupon is required %s in the cart', 'traveler' ), implode( ', ', $products_name ) )
                        ];
                    }
                }
                ...
                if ( $exclude_products ) {
                    if($exclude_products_post_type != '-1') {
                        ...
                        if (!empty($check)) {
                            return [
                                'status' => 0,
                                'message' => sprintf(__('This coupon is required %s NOT in the cart', 'traveler'), implode(', ', $products_name))
                            ];
                        }
                    }
                }else{
                    if($exclude_products_post_type != '-1') {
                        ...
                        if ($exclude_products_post_type == $current_post_type) {
                            return [
                                'status' => 0,
                                'message' => sprintf(__('This coupon is required of %s service NOT in the cart', 'traveler'), $obj_post_type_name)
                            ];
                        }
                    }
                }
                ...
                if ( $limit_per_coupon ) {
                    ...
                    if ( $count_used >= $limit_per_coupon ) {
                        return [
                            'status'  => 0,
                            'message' => __( 'This coupon is reach the limit of usage', 'traveler' )
                        ];
                    }
                }
                ...
                if ( $limit_per_user and ( is_user_logged_in() ) ) {
                    ...
                    if ( $query->found_posts >= $limit_per_user ) {
                        return [
                            'status'  => 0,
                            'message' => __( 'This coupon is reach the limit of usage per user', 'traveler' )
                        ];
                    }
                }
            ...
            static function _check_in_items( $post_id, $cart_items )
            {
                foreach ( $cart_items as $key => $value ) {
                    if($key === 'car_transfer'){
                        $id = isset($value['data']['car_id']) ? $value['data']['car_id'] : 0;
                        if($post_id == $id){
                            return true;
                        }
                    }

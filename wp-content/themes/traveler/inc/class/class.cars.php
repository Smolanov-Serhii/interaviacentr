    ...
    class STCars extends TravelerObject {
        ...
        public function st_filter_cars_ajax() {
            ...
            if ($wp_query->max_num_pages < $paged) {
                $result = array(
                    'content' => '<h3 class="ajax-filter-not-found">' . __('No car found', 'traveler') . '</h3>',
                    'pag' => '',
                    'count' => '',
                    'page' => 1
                );
            ...
            if (!$query->found_posts) {
                echo '<h3 class="ajax-filter-not-found">' . __('No car found', 'traveler') . '</h3>';
            }
                                ...
                                if (is_array($field_name)) {
                                    foreach ($field_name as $v) {
                                        if (!STInput::request($v)) {
                                            $message .= sprintf(__('%s is required', 'traveler'), $value['title']) . '<br>';
                                            $pass_validate = false;
                                        }
                                    }
            ...
            if (isset($_REQUEST['location_id_pick_up']) && !empty($_REQUEST['location_id_pick_up']) && isset($_REQUEST['location_id_drop_off']) && !empty($_REQUEST['location_id_drop_off'])) {
                ...
                if (!empty($location_without_s)) {
                    if (!in_array($location_id_pick_up, $location_without_s)) {
                        STTemplate::set_message(__('The pickup location is not match with car location.', 'traveler'), 'danger');
                        $pass_validate = false;
                ...
                //if (!$in_location) {
                //    STTemplate::set_message(sprintf(__('This car is not lease in %s.', 'traveler'), get_the_title($location_id_pick_up)), 'danger');
                //    $pass_validate = false;
                //   return false;
                //}
                if (!$pickup_country) {
                    STTemplate::set_message(__('The \'country\' field not set for the \'' . get_the_title($location_id_pick_up) . '\'', 'traveler'), 'danger');
                    $pass_validate = false;

                    return false;
                }
                if (!$dropoff_country) {
                    STTemplate::set_message(__('The \'country\' field not set for \'' . get_the_title($location_id_drop_off) . '\'', 'traveler'), 'danger');
                    $pass_validate = false;
        ...
        function _get_query_clauses($clauses) {
            if (STAdminCars::check_ver_working() == false)
                return $clauses;
            $post_type = get_query_var('post_type');

...
if (!class_exists('st_categories')) {
...

    {
...

        {
            ...
            parent::__construct('st_categories', __("ST Categories", 'traveler'), $widget_ops);
...

        {
            ...
            <p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title:'); ?> <input
            ...
            <p>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php echo __("Header style", 'traveler'); ?></label>

                <select id="<?php echo esc_attr($this->get_field_id('style')); ?>"

                        name="<?php echo esc_attr($this->get_field_id('style')); ?>">

                    <option value="" <?php if (isset($instance['style']) && $instance['style'] == "") echo "selected"; ?>>

                        Default

                    </option>

                    <option value="tour_box" <?php if (isset($instance['style']) && $instance['style'] == "tour_box") echo "selected"; ?>><?php echo __("Tour box style", 'traveler'); ?></option>
            ...
            <p>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>"> <?php echo __("Choose a category", 'traveler'); ?>
                            ...
                            if (!empty($taxonomies) and is_array($taxonomies)) {
                                ...
                                foreach ($taxonomies as $key => $val) {
                                    ...
                                    <input type="radio"
                                           ...
                                           id="<?php echo esc_attr($this->get_field_id('post_type')) . $val->labels->name; ?>"

                                           name="<?php echo esc_attr($this->get_field_id('post_type')); ?>"

                                           value="st_hotel/<?php echo esc_attr($key); ?>" <?php if ($instance['post_type'] == 'st_hotel/' . $key) {
                            ...
                            if (!empty($taxonomies) and is_array($taxonomies)) {
                                ...
                                foreach ($taxonomies as $key => $val) {
                                    ...
                                    <input type="radio"
                                           ...
                                           value="st_rental/<?php echo esc_attr($key); ?>" <?php if ($instance['post_type'] == 'st_rental/' . $key) {
                            ...
                            if (!empty($taxonomies) and is_array($taxonomies)) {
                                ...
                                foreach ($taxonomies as $key => $val) {
                                    ...
                                    <input type="radio"
                                           ...
                                           value="st_cars/<?php echo esc_attr($key); ?>" <?php if ($instance['post_type'] == 'st_cars/' . $key) {
                                    ...
                                    echo "<label for='" . esc_attr($this->get_field_id('post_type') . $val->labels->name ). "''>" . $val->labels->name . "</label><br>";
                            ...
                            if (!empty($taxonomies) and is_array($taxonomies)) {
                                ...
                                foreach ($taxonomies as $key => $val) {
                                    ...
                                    <input type="radio"
                                           ...
                                           value="st_tours/<?php echo esc_attr($key); ?>" <?php if ($instance['post_type'] == 'st_tours/' . $key) {
                            ...
                            if (!empty($taxonomies) and is_array($taxonomies)) {
                                ...
                                foreach ($taxonomies as $key => $val) {
                                    ...
                                    <input type="radio"
                                           ...
                                           value="st_activity/<?php echo esc_attr($key); ?>" <?php if ($instance['post_type'] == 'st_activity/' . $key) {
...

        {
            ...
            if (!empty($_POST[$this->get_field_id('post_type')])) {

                $new_instance['post_type'] = $_POST[$this->get_field_id('post_type')];

            }

            if (!empty($_POST[$this->get_field_id('style')])) {

                $new_instance['style'] = $_POST[$this->get_field_id('style')];

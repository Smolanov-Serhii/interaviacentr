...
<div class="div_item_map <?php echo 'div_map_item_'.get_the_ID() ?>" >
    ...
    <div class="thumb item_map">
            ...
            <div class="booking-item-img-wrap st-popup-gallery">
                <a href="<?php echo (!empty($thumb_obj[0]))?$thumb_obj[0]:'#' ?>" class="st-gp-item">
                    ...
                    if(has_post_thumbnail() and get_the_post_thumbnail()){
                        the_post_thumbnail(array(240, 120, 'bfi_thumb' => TRUE), array('alt' => TravelHelper::get_alt_image(get_post_thumbnail_id( )))) ;
                    }else {
        ...
        <div class="thumb-caption">
                            ...
                            if($key != 'st_cars_pickup_features') {
                                ...
                                if(!empty($data_term) and is_array($data_term)){
                                    foreach($data_term as $k=>$v){
                                        ...
                                        if($i<$limit){
                                            if($dk_check == true){
                                                echo  '<li title="" data-placement="top" rel="tooltip" data-original-title="'.esc_attr($data_title_info).'">
                                                                        <i class="'.TravelHelper::handle_icon(get_tax_meta($v->term_id, 'st_icon',true)).'"></i>
                                                                        <span class="booking-item-feature-sign">'.esc_attr($data_info).'</span>
                                                                    </li>';
                                            }else{
            ...
            <a class="btn btn-primary btn_book" href="<?php echo esc_attr($link)?>"><?php _e("Book Now",'traveler') ?></a>
            <button class="btn btn-default pull-right close_map" onclick="closeGmapThumbItem(this)" ><?php _e("Close",'traveler') ?></button>
        </div>
    </div>
    <div class="gap"></div>
</div>

<!--<script>jQuery(function($){$('.st-popup-gallery').each(function () {$(this).magnificPopup({delegate: '.st-gp-item', type: 'image', gallery: {enabled: true}});})})</script>-->
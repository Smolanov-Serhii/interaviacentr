            ...
            if($key != 'st_cars_pickup_features') {
                ...
                if(!empty($data_term) and is_array($data_term)){
                    foreach($data_term as $k=>$v){
                        ...
                        if($i<$limit){
                            if($dk_check == true){
                                $data_text_lg .=  '<li title="" data-placement="top" rel="tooltip" data-original-title="'.esc_attr($data_title_info).'">
...
<ul class="booking-item-features booking-item-features-small clearfix"><?php echo balanceTags($data_text_lg)?></ul>
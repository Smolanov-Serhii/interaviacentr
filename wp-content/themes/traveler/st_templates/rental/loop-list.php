...
<li <?php post_class('booking-item') ?> itemscope itemtype="http://schema.org/Product">
    ...
    <div class="row">
            ...
            if(!empty($custom_feild) and is_array($custom_feild)):?>
                ...
                foreach($custom_feild as $key=>$value):
                    if($value['show_in_list']=='on'):
                        echo '<li rel="tooltip" data-placement="top" title="" data-original-title="'.esc_attr($value['title']).'"><i class="'.esc_attr(TravelHelper::handle_icon($value['icon'])).'"></i>';
                        ...
                        if($meta){
                            echo '<span class="booking-item-feature-sign">x '.esc_html($meta).'</span>';
                        }
                    endif;
        ...
        ?>
        <div class="col-md-3">
            <span class="booking-item-price-from"><?php _e("Price",'traveler') ?></span>
            <?php
            if($is_sale):
            ...
            <?php if($show_price == 'on' || $price) : ?>
                <span
                    class="booking-item-price"><?php echo TravelHelper::format_money($price) ?></span><span>/<?php printf(__(' %d night(s)', 'traveler'), $numberday); ?></span><br>
            <?php endif; ?>

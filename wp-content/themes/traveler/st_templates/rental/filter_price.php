...
<?php if(empty($hidde_button)){ ?><div><form method="get" action=""><?php }else { ?><div class=" form-group form-group-<?php echo esc_attr($field_size) ?> "><?php }?>
    ...
    if ($data['title']){
        echo  "<label>".esc_html($data['title'])."</label>";
        }
    }
    ...
    echo '<input name="price_range" type="text" value="'.esc_attr($value_show).'" class="price-slider" data-symbol="'.esc_attr(TravelHelper::get_current_currency('symbol')).'" data-min="'.esc_attr($min).'" data-max="'.esc_attr($max).'" data-step="'.esc_attr(st()->get_option('search_price_range_step',0)).'">';

    ?>
    <?php if(empty($hidde_button)){ ?>
    <button style="margin-top: 4px;" type="submit" class="btn btn-primary"><?php st_the_language('rental_filter')?></button>
    <?php } ; ?>

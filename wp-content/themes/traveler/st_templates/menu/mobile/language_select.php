    ...
    if(!empty($langs))
            {
                foreach($langs as $key=>$value){
                    if($value['active']==1){
                        $current=$value['native_name'].' <img  height="12px" width= "18px" src="'.esc_url($value['country_flag_url']).'" alt="'.esc_attr($value['native_name']).'" title="'.esc_attr($value['native_name']).'">';
                        break;
                    }
                }
                ?>
                ...
                foreach ( $variable as $key => $value) {
                    ...
                    <li>
                        <a title="<?php echo esc_attr($lang_name) ; ?>" href="<?php echo esc_attr($qtrans_link);?>">
                            <img src="<?php echo esc_url($flag_dir.$flags[$value]) ; ?>" alt="<?php echo esc_attr($lang_name) ; ?>" title="<?php echo esc_attr($lang_name) ; ?>">

...
<li class="<?php echo esc_attr( $class ); ?>">
	...
	<div class="traveler-cart-mini">
            ...
            <?php
                if($booking_fee_price > 0) {
                    ...
                    <div class="col-lg-12 traveler-cart-booking-fee">
                        <b><?php echo __('Booking fee: ', 'traveler'); ?></b>
					...
					<div class="row">
						...
						<div class="col-xs-8">
							...
							<h4 class="traveler-cart-price">
								<?php echo __('Price', 'traveler' ); ?>: <span><?php echo TravelHelper::format_money( $price ); ?></span>
							</h4>
						</div>
					</div>
				</div>
		...
		<div class="traveler-cart-footer clearfix">
			<a href="<?php echo esc_url( $cart_url ); ?>" class="text-center pull-left"><?php echo __('Checkout', 'traveler' ); ?></a>
			<a href="<?php echo add_query_arg(array('action' => 'st-remove-cart', 'security' => wp_create_nonce( 'st-security' ))); ?>" class="minicart-remove pull-right"><?php echo __('Remove Cart', 'traveler'); ?></a>
		</div>
	</div>
	<?php endif; ?>
</li>
		...
		<?php 
			...
			switch ( $post_type) {
			 ...
			 if(st()->get_option('booking_modal','off')=='on'){
			 	...
			 	$attr = ' data-effect="mfp-zoom-out" onclick="return false" data-target="#'. esc_attr($prefix) .'_booking_'. esc_attr($post_id_global) .'" ';
			 }
		?>
			<a href="<?php echo esc_url( $check_out_url ); ?>" class="text-center <?php echo esc_attr($class); ?> pull-left" <?php echo esc_attr($attr); ?>><?php echo __('Checkout', 'traveler' ); ?></a>
			<a href="<?php echo add_query_arg(array('action' => 'st-remove-cart', 'security' => wp_create_nonce( 'st-security' ))); ?>" class="minicart-remove pull-right"><?php echo __('Remove Cart', 'traveler'); ?></a>
		</div>
	</div>
	<?php endif; ?>

...
<header id="menu3" class="st_menu" >
    <div id='top_header' class="header-top <?php echo apply_filters('st_header_top_class','') ?>">
                        ...
                        <?php if(has_nav_menu('primary')){
                            wp_nav_menu(
                                array(
                                    ...
                                    "container"=>"",
                                    'items_wrap'      => '<ul id="slimmenu" data-title="<a href=\''.esc_url(home_url('/')).'\'><img alt=\''.TravelHelper::get_alt_image().'\' width=auto height=40px class=st_logo_mobile src='.esc_url($logo_url).' /></a>" class="%2$s slimmenu">%3$s</ul>',
        ...
        <form class="main-header-search" action="<?php echo home_url( '/' ); ?>" method="get">
            <div class="container">
                <input type="text" data-lang="<?php echo (defined('ICL_LANGUAGE_CODE'))?ICL_LANGUAGE_CODE:false; ?>" name="s" value="<?php echo get_search_query() ?>" class="form-control st-top-ajax-search" placeholder="<?php echo __('Search...', 'traveler'); ?>">

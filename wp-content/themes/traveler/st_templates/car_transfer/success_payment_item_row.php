...
<tr>
    ...
    <td>
        ...
        <?php if($use_agent_info){ ?>
            ...
            <p style="margin-top:10px;"><strong> <?php echo __('Journey', 'traveler') ?>:</strong> <?php echo esc_html($transfer_from . ' - '. $transfer_to); ?></p>
        ...
        <?php } ?>
        <?php endif;?>
        <p><strong><?php echo __('Arrival Date', 'traveler') ?>:</strong>
        ...
        <p><strong><?php echo __('Departure Date', 'traveler') ?>:</strong>
	        <?php echo date(TravelHelper::getDateFormat(), strtotime($check_out)); ?>
	        <?php echo date(' H:i:s', strtotime($check_out_time)); ?>
        </p>
        <?php } ?>
        <p><strong><?php echo __('Passengers', 'traveler') ?>:</strong> <?php echo esc_html($passenger); ?></p>
        <?php
            $time = $data['data']['distance'];
            if(!empty($time)):
        ?>
        <p><strong><?php echo __('Estimated Time', 'traveler') ?></strong>
            <?php
        ...
        if(!empty($extras) and is_array($extras)){
            ?>
            <p><strong><?php echo __('Extra services', 'traveler') ?></strong></p>
            <?php
            echo '<p>';

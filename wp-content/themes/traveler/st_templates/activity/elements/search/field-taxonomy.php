...
$default=array(
    ...
    'taxonomy'=>'',
    'is_required'=>'on',
    'type_show_taxonomy_activity'=>'checkbox'
);
...
if (!empty($st_direction) and $st_direction == 'vertical') { $checkbox_item_size = apply_filters("st_taxonomy_checkbox_size" , '6'); } // from 1.2.1
    ...
    if($type_show_taxonomy_activity == "select"){
        ?>
        <div class="form-group form-group-<?php echo esc_attr($field_size)?>" taxonomy="<?php echo esc_html($taxonomy) ?>">
            <label for="field-activity-<?php echo esc_html($taxonomy) ?>"><?php echo esc_html( $title)?></label>
            <?php
            $args = array(
                'show_option_none' => __( '-- Select --' , 'traveler' ),
                'option_none_value' => "",
                'hierarchical'      => 1 ,
                'name'              => 'taxonomy['.$taxonomy.']' ,
                'class'             => 'form-control' ,
                'id'             => 'field-activity-tax-'.$taxonomy ,
                'taxonomy'          => $taxonomy ,
            );
    ...
    <?php }else{ ?>
        <div class="form-custom-taxonomy form-group form-group-<?php echo esc_attr($field_size)?>" taxonomy="<?php echo esc_html($taxonomy) ?>">
            <label for="field-taxonomy-<?php echo esc_html($taxonomy) ?>"><?php echo esc_html( $title)?></label>
            <div class='row'>

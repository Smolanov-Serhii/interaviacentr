...
?>
<h3><?php esc_html_e('Search for Activity','traveler') ?></h3>
<form role="search" method="get" class="search main-search" action="<?php echo home_url( '/' ); ?>">
                            ...
                            if(!empty($fields))
                            {
                                foreach($fields as $key=>$value)
                                {
                                    ...
                                    <div class="<?php echo esc_attr($size_class); ?>">
                                        <?php echo st()->load_template('activity/elements/search/field',$name,array('data'=>$value,'field_size'=>$field_size,'location_name'=>'location_name','placeholder'=>$value['placeholder'])) ?>
                                    </div>
                                <?php
                                }
                            }?>
    ...
    <?php endif;  ?>
    <button class="btn btn-primary btn-lg" type="submit"><?php esc_html_e('Search for Activity','traveler') ?></button>
</form>

...
<div class="form-group st-field-<?php echo esc_attr($data['type']); ?>">
    ...
    if(!empty($arr_img)){
        foreach ($arr_img as $k => $v){
            ?>
            <div class="item" style="background-image: url('<?php echo esc_attr($v); ?>')"><div class="del" data-id="<?php echo esc_attr($k); ?>" data-url="<?php echo esc_attr($v); ?>" data-output="<?php echo esc_attr($data_output); ?>"></div></div>
            <?php
        }
    }
    ?>
    </div>
    <div class="st-upload" data-multi="<?php echo esc_attr($multi); ?>" data-output="<?php echo esc_attr($data_output); ?>">
        <div class="inner">
            <span class="add">+</span>
            <span class="text"><?php echo __('Upload', 'traveler'); ?></span>
        </div>

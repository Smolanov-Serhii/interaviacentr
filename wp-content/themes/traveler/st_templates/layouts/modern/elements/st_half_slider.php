...
<div class="st-half-slider-wrapper clearfix">
    <div class="st-half-slider-text has-matchHeight has-gutterLeft">
        <div class="text-wrapper">
            ...
            if (!empty($link) && $link != '|||') {
                echo '<a class="btn btn-primary btn-md" href="' . esc_url($link['url']) . '" target="' . esc_attr($link['target']) . '">' . esc_html($link['title']) . '</a>';
            }
            ?>
        </div>
    </div>
        ...
        if (!empty($gallery) && is_array($gallery)) { ?>
            <div class="st-owl-carousel owl-carousel">
                ...
                foreach ($gallery as $galleryID) {
                    ...
                    $class = Assets::build_css('background-image: url(' . esc_url($img) . ');');
                    echo '<div class="item ' . esc_attr($class) . '"></div>';
                }
                ?>
            </div>
            <?php
        ...
        <div class="search-form clearfix">
            <form action="<?php echo esc_url(get_the_permalink($result_page)); ?>" class="form" method="get">
                ...
                <input type="submit" class="btn btn-primary" name="submit"
                       value="<?php echo __('Find a Tour', 'traveler'); ?>">
            </form>
        </div>
    </div>
</div>

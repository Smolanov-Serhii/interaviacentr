...
<div class="list-group  st-gallery-grid poup-galler-st">
    	<?php
		    foreach( $img_ids as $count=>$im_id ){
		      ...
		      $tes_title= !empty($meta['title']) ? $meta['title'] : '';
		      $tes_caption=!empty($meta['caption']) ? $meta['caption'] : '';
			?>
	        <div class="item <?php if($count == 0){ ?> bg-icon-st <?php } ?>" <?php if($count != 0){ ?> style="display: none;" <?php } ?>>
	            <div class="thumbnail">
	                <a class="galleryItem" href="<?php echo esc_url($image_src[0]);?>" data-group="grid" title=""><img class="group list-group-image" src="<?php echo esc_url($image_src[0]);?>" alt="<?php echo esc_attr($tes_title);?>" style="min-height: 450px;" /></a>
	            </div>
	        </div>
	    <?php }?>

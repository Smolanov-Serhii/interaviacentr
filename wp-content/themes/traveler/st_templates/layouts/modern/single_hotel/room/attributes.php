    ...
    if(!empty($tax_in_room)){
        foreach ($tax_in_room as $k => $v){
            ...
            if($obj) {
                ...
                if (!empty($terms)) {
                    ?>
                    <div class="<?php echo esc_attr($div_col) ?>">
                        <h3 class="section-title sts-pf-font"><?php echo esc_html($label); ?></h3>
                        <ul class="row">
                            <?php
                            foreach ($terms as $kk => $vv) {
                                $icon = TravelHelper::getNewIcon('check-1', '#1A2B48', '20px', '20px', true);
                                echo '<li class="col-xs-6 has-matchHeight">' . balanceTags($icon). esc_html($vv->name) . '</li>';
                            }
                            ?>
                        </ul>
                    </div>

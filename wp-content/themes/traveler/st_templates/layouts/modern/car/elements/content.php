...
<div class="col-lg-9 col-md-9">
    <?php echo st()->load_template('layouts/modern/hotel/elements/toolbar', '', array('style' => $style, 'format' => $format, 'layout' => $layout, 'service_text' => __('New car', 'traveler'), 'post_type' => 'st_cars')); ?>
    <div id="modern-search-result" class="modern-search-result" data-layout="1">
        ...
        if($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                echo st()->load_template('layouts/modern/car/elements/loop/' . esc_attr($style));
            }
        }else{

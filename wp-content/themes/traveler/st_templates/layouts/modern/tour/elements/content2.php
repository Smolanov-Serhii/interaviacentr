...
if(empty($layout))
    $layout = '';
?>
    <?php //echo st()->load_template('layouts/modern/tour/elements/toolbar-with', '', array('style' => $style, 'format' => $format, 'layout' => $layout, 'service_text' => __('New tour', 'traveler'), 'post_type' => 'st_tours', 'top_search' => true)); ?>
    <div class="toolbar">
        ...
        <div class="dropdown-menu sort-menu sort-menu-mobile">
            ...
            <div class="sort-item st-icheck">
                <div class="st-icheck-item"><label> <?php echo __('New tour', 'traveler'); ?><input class="service_order" type="radio" name="service_order_m_<?php echo esc_attr($format); ?>" data-value="new" /><span class="checkmark"></span></label></div>
            </div>
            <div class="sort-item st-icheck">
        ...
        <h3 class="search-string modern-result-string" id="modern-result-string"><?php echo balanceTags(STTour::get_instance()->get_result_string()); ?> <div id="btn-clear-filter" class="btn-clear-filter" style="display: none"><?php echo __('Clear filter', 'traveler'); ?></div> </h3>
    </div>
<?php
    ...
    <div id="modern-search-result" class="modern-search-result">
        ...
        if($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                echo st()->load_template('layouts/modern/tour/elements/loop/' . esc_attr($style), '', array('top_search' => true));
            }
        }else{

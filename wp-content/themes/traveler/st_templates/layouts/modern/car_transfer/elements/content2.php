...
?>
    <div class="toolbar">
        ...
        <h3 class="search-string modern-result-string" id="modern-result-string"><?php echo balanceTags(STCarTransfer::get_instance()->get_result_string()); ?> <div id="btn-clear-filter" class="btn-clear-filter" style="display: none"><?php echo __('Clear filter', 'traveler'); ?></div> </h3>
    </div>
<?php
    ...
    <div id="modern-search-result" class="modern-search-result">
        ...
        if($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                echo st()->load_template('layouts/modern/car/elements/loop/' . esc_html($style), '', array('top_search' => true));
            }
        }else{

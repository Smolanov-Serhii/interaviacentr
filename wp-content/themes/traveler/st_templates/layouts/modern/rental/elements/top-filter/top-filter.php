...
<div class="top-filter">
    <ul>
        ...
        if(!empty($filters)){
            foreach ($filters as $k => $v){
                echo st()->load_template('layouts/modern/rental/elements/top-filter/' . esc_html($v['rs_filter_type']), '', array('title' => $v['title'], 'taxonomy' => $v['rs_filter_type_taxonomy']));
            }
        }
        ?>
    </ul>
    <?php if($format != 'popup'){ ?>
    <div class="show-map">
        <span><?php echo __('Maps', 'traveler'); ?></span>
        <label class="switch">

...
?>
<h4><?php if (!$title){echo __("Largest Selection" , 'traveler') ; }else {echo esc_html($title) ; }?></h4>
<p>
	<?php 
		if (!empty($post_type) and is_array($post_type) ){	
			...
			foreach ($post_type as $key => $value) {
				...
				if (STLocation::round_count_reviews($result['offers'])){
					echo "<br>";
					echo esc_html(STLocation::round_count_reviews($result['offers'])." ".$result['post_type_name']);
				}				
	...
	<?php if (!empty($count_review_s) and $count_review_s =="on" and STLocation::round_count_reviews($count_review)){?>
		<br><?php echo __("Over",'traveler')  ;?> <?php echo esc_attr(STLocation::round_count_reviews($count_review)) ;?> <?php if ($count_review >=2){echo __("reviews" , 'traveler') ;}else{echo __("review" , 'traveler') ;}?>
	<?php }?>

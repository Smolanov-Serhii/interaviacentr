...
if(have_posts()) {
    while( have_posts() ) {
        ...
        if($st_location_style == 'list') {
	        ...
	        $return .= st()->load_template( 'rental/loop', 'grid' );
        }
    }
} else {
    echo '<div class="col-xs-12"><div class="alert alert-warning">' . __( "There are no available rentals for this location, time and/or date you selected." , 'traveler' ) . '</div></div>';
}
if($st_location_style == "list") {
    $return .= '</ul>';
} else {

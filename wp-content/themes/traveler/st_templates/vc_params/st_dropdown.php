...
switch ( $stype ) {
    ...
    case 'list_post_type':
	    ...
	    $list = array();
	    $list = array( __( '--Select--' , 'traveler' ) => '' );
	    if(!empty( $list1 ) and is_array( $list1 )) {
		    foreach( $list1 as $kk => $vv ) {
			    if($vv == 'st_cars') {
				    $list[ __( 'Car' , 'traveler' ) ] = $vv;
			    }
			    if($vv == 'st_tours') {
				    $list[ __( 'Tour' , 'traveler' ) ] = $vv;
			    }
			    if($vv == 'st_hotel') {
				    $list[ __( 'Hotel' , 'traveler' ) ] = $vv;
			    }
			    if($vv == 'st_rental') {
				    $list[ __( 'Rental' , 'traveler' ) ] = $vv;
			    }
			    if($vv == 'st_activity') {
				    $list[ __( 'Activity' , 'traveler' ) ] = $vv;
			    }
			    if($vv == 'hotel_room') {
				    $list[ esc_html__('Room','traveler')] = $vv;
			    }
		    }
	    }
        break;
    ...
    <div class="edit_form_line st_checkbox_item">
        <div class="ui-widget">
			...
			if ( ! empty( $list ) ) {
				...
				echo '<select name="' . esc_attr($param_name) . '" class="' . esc_attr($class_auto) . ' wpb_vc_param_value wpb-input wpb-select number_post dropdown 2 ' . esc_attr( $class ) . ' "
                        data-option="2">';
				switch ( $stype ) {
					case 'tax':
                    case 'list_tax':

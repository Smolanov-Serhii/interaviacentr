 ...
 */
    ...
    wp_enqueue_script( 'filter-ajax-hotel-room.js');
                ...
                <?php
                        ...
                        if(!empty($st_orderby)) {
                            ...
                            if (!in_array($active, $orderby_key)) {
                                echo '<li class="active" style="display: none !important;"><a class="checkbox-filter-ajax" data-type="order" data-value="' . esc_attr($active) . '"></a>';
                            }
                        }else{
                            echo '<input type="hidden" id="jhotelroom-orderby" value="-1" />';
                            $active = STInput::request('orderby');
                        }
                        echo '<input type="hidden" id="jhotelroom-category" value="'. esc_attr($taxonomy) .'" />';
                        if(!empty($allOrderby) and is_array($allOrderby)):
                            foreach($allOrderby as $key=>$value)
                            {
                                ...
                                if($active == $key){
                                    echo '<li class="active"><a href="'.esc_url($link).'" class="checkbox-filter-ajax" data-type="order" data-value="' . esc_attr($key) . '">'.balanceTags($value['name']).'</a>';
                                }elseif($key == 'new' and empty($active)){
                                    echo '<li class="active"><a href="'.esc_url($link).'" class="checkbox-filter-ajax" data-type="order" data-value="' . esc_attr($key) . '">'.balanceTags($value['name']).'</a>';
                                }else{
                                    echo '<li><a href="'.esc_url($link).'" class="checkbox-filter-ajax" data-type="order" data-value="' . esc_attr($key) . '">'.balanceTags($value['name']).'</a>';
...
<div class="ajax-filter-cover">
    <div class="ajax-filter-loading">
        <img src="<?php echo ST_TRAVELER_URI; ?>/img/loading-filter-ajax.gif"
             alt="<?php echo __('Loading tours', 'traveler'); ?>"/>
    </div>

...
?>
<h3><?php _e("Search for Room",'traveler')?></h3>
    ...
    <div class="row">
        ...
        if(!empty($fields))
        {
            foreach($fields as $key=>$value)
            {
                ...
                ?>">
                    <?php echo st()->load_template('hotel-room/elements/search/field_'.esc_html($name),false,array('data'=>$value)) ?>
                </div>
            <?php
            }
        }?>
                ...
                <div class="<?php  if($st_direction=='horizontal') echo 'row';?>">
                    ...
                    if(!empty($fields))
                    {
                        foreach($fields as $key=>$value)
                        {
                            ...
                            ?>
                            <div class="col-md-<?php echo esc_attr($size);  ?>">
                                <?php echo st()->load_template('hotel-room/elements/search/field_'.esc_html($name),false,array('data'=>$value,'field_size'=>$field_size,'location_name'=>'location_name')) ?>
                            </div>
                        <?php
                        }
                    }?>

            ...
            if(!empty($fields))
            {
                foreach($fields as $key=>$value)
                {
                    ...
                    <div class="<?php echo esc_attr($size_class); ?>">
                        <?php echo st()->load_template('car_transfer/elements/search/field-'.esc_html($name),false,array('data'=>$value,'field_size'=>$field_size,'location_name'=>'location_name','placeholder'=>$value['placeholder'],'st_direction'=>$st_direction)) ?>
                    </div>
                    <?php
                }
            }?>
        </div>
        <button class="btn btn-primary btn-lg" type="submit"><?php echo __('Search', 'traveler'); ?></button>
    </form>

...
<div class="row">
    <div class="col-md-12">
        ...
        echo '
        <div class="row">
            <div class="col-md-3"><h3>'.esc_html($obj->labels->singular_name).'</h3></div>
            <div class="col-md-9 text-right">
                <h3>'.balanceTags($rs_string).'</h3>
            </div>
        </div>
        ';
        ...
        <?php
        if($st_style == '1'){
            echo '<ul class="booking-list loop-cars style_list">'.balanceTags($content).'</ul>';
        }
        if($st_style == '2'){
            echo '<div class="row row-wrap isotope-container">'.balanceTags($content).'</div>';
        }
        ?>
        <div class="row">
            <div class="col-md-12 text-right">
                <?php  if($query->found_posts > $st_number){ ?>
                    <a href="<?php  echo esc_url($link_view_all) ?>"><?php _e("View All",'traveler') ?> <?php echo esc_html($obj->labels->singular_name)?></a>
                <?php } ?>
            </div>
            <div class="col-sm-12">
                <hr>

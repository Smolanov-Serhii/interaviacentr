...
?>
    ...
    <form role="search" method="get" class="search main-search" action="<?php echo esc_url($link_action) ?>">
            ...
            if(!empty($fields))
            {
                foreach($fields as $key=>$value)
                {
                    ...
                    <div class="<?php echo esc_attr($size_class); ?>">
                        <?php echo st()->load_template('hotel-room/elements/search/field_'.esc_html($name),false,array('data'=>$value,'field_size'=>$field_size,'location_name'=>'location_name','placeholder'=>$value['placeholder'],'st_direction'=>$st_direction)) ?>
                    </div>
                    <?php
                }
            }?>
                ...
                <div class="row">
                    ...
                    if(!empty($fields))
                    {
                        foreach($fields as $key=>$value)
                        {
                            ...
                            ?>
                            <div class="<?php echo esc_attr($size_class); ?>">
                                <?php echo st()->load_template('hotel-room/elements/search/field_'.esc_html($name),false,array('data'=>$value,'field_size'=>$field_size,'location_name'=>'location_name','st_direction'=>$st_direction)) ?>
                            </div>
                        <?php
                        }
                    }?>
        ...
        <button class="btn btn-primary btn-lg" type="submit"><?php _e("Search for Room",'traveler')?></button>
    </form>

...
?>
    ...
    <form role="search" method="get" class="search main-search" action="<?php echo esc_url($link) ?>" target="_blank">
            ...
            if(!empty($fields))
            {
                foreach($fields as $key=>$value)
                {
                    ...
                    <div class="<?php echo esc_attr($size_class); ?>">
                        <?php echo st()->load_template('flight/elements/search/field_'.esc_html($name),false,array('data'=>$value,'field_size'=>$field_size,'location_from'=>'a1','location_to'=>'a2','placeholder'=>$value['placeholder'],'st_direction'=>$st_direction, 'is_required' => $value['is_required'])) ?>
                    </div>
                    <?php
                }
            }?>
        ...
        <button class="btn btn-primary btn-lg" type="submit"><?php echo __('Search for Flight', 'traveler'); ?></button>
    </form>

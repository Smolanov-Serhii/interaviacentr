...
<ul class="list list-inline text-small">
	<?php if($use_agent_info){?>
		...
		if(!empty($email))
			echo '<li><a href="mailto:'.esc_url($email).'"><i class="fa fa-envelope"></i> '.__('Agent E-mail','traveler').'</a> </li>';
		?>
		<?php if($website=get_the_author_meta('user_url',$user_id)):?>
			<li><a target="_blank" href="<?php echo esc_url( $website );?>"> <i class="fa fa-home"></i> <?php _e('Agent Website','traveler');?></a>
			</li>
		<?php endif;?>
		<?php
		$phone = get_user_meta($user_id,'st_phone',true);
		if(!empty($phone))
			echo '<li><a href="tel:'.str_replace(" ","",trim($phone)).'"><i class="fa fa-phone"></i> '.esc_html($phone).'</a></li>';
		?>
	...
	<?php }else {?>
		...
		if(!empty($email))
			echo '<li><a href="mailto:'.esc_attr($email).'"><i class="fa fa-envelope"></i> '.__('Agent E-mail','traveler').'</a> </li>';
		?>

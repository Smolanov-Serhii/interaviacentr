        ...
        if(!empty($fields)){
            foreach($fields as $key=>$value){
                ...
                <div class="<?php echo esc_attr($size_class); ?>">
                    <?php 
                    if($value['field_atrribute'] == 'location'){
                        
                        if(is_singular('st_cars')){
                            echo st()->load_template('cars/elements/search/field-'.esc_html($value['field_atrribute']) .'-single',false,array('data'=>$value)) ;
                        } else {
                            echo st()->load_template('cars/elements/search/field-'.esc_html($value['field_atrribute']),false,array('data'=>$value)) ;
                        }
                    } else {
                        echo st()->load_template('cars/elements/search/field-'.esc_html($value['field_atrribute']),false,array('data'=>$value)) ;
                    }

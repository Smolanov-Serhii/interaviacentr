...
<form method="get" action="<?php the_permalink() ?>" class="main-search">
    <div class="row">
        <?php
        ...
        if(!empty($fields)){
            foreach($fields as $key=>$value){
                ...
                $size_class = " col-md-".esc_attr($size)." col-lg-".esc_attr($size). " col-sm-12 col-xs-12 " ;
                ?>
                <div class="<?php echo esc_attr($size_class); ?>">
                    <?php echo st()->load_template('cars/elements/search/field-'.esc_html($value['field_atrribute']),false,array('data'=>$value)) ?>
                </div>
        <?php
            }
        }
                ...
                <div class="<?php  if($st_direction=='horizontal') echo 'row';?>">
                    ...
                    if(!empty($fields)){
                        foreach($fields as $key=>$value){
                            ...
                            ?>
                            <div class="<?php echo esc_attr($size_class); ?>">
                                <?php echo st()->load_template('cars/elements/search/field-'.esc_html($value['field_atrribute']),false,array('data'=>$value,'field_size'=>$field_size,'st_direction'=>$st_direction,'location_name'=>'location_name','placeholder'=>$value['placeholder'])) ?>
                            </div>
                        <?php
                        }
                    }

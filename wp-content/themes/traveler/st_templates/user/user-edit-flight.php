 ...
 *
 */
$hotel_add_fields = ST_Config::get('partner.add.flight');
...
<div class="st-create-service">
    <h2 class="st-header">
        <?php
        if(!empty($post_id)) {
            echo __('Edit Flight', 'traveler');
        }else{
            echo __('Add Flight', 'traveler');
        }
        ?>
    </h2>
            ...
            <ul class="nav nav-tabs" role="tablist">
                ...
                if(!empty($hotel_add_fields['tabs'])){
                    foreach ($hotel_add_fields['tabs'] as $k => $v){
                        ...
                        echo '<li role="presentation" class="'. esc_attr($class_active) .'" data-obj="tab"><a href="#'. esc_attr($v['name']) .'" aria-controls="'. esc_attr($v['name']) .'" role="tab" data-toggle="tab">'. esc_attr($v['label']) . balanceTags($icon) .'</a></li>';
                    }
                }
                ?>
            </ul>
                                ...
                                <form method="post" action="" class="st-partner-create-form">
                                    ...
                                    if(!empty($fields)){
                                        foreach ($fields as $kk => $vv){
                                            $class_col = 'col-lg-12';
                                            if(!empty($vv['col']))
                                                $class_col = 'col-lg-' . esc_attr($vv['col']);
                                            ...
                                            <div class="<?php echo esc_attr($class_col); ?> st-partner-field-item">
                                                <?php echo st()->load_template('fields/' . esc_attr($vv['type']), '', array('data' => $vv, 'post_id' => $post_id)); ?>
                                            </div>
                                            <?php
                                        }
                                    }
                                    ...
                                    if(isset($_GET['id'])&& !empty($_GET['id'])){
                                        echo '<input type="hidden" name="btn_update_post_type_flight" value="1"/>';
                                        echo '<input type="hidden" name="post_id" value="'. esc_attr($_GET['id']) .'"/>';
                                    }else{
                                        echo '<input type="hidden" name="btn_insert_post_type_flight" value="1"/>';
                                        echo '<input type="hidden" class="st-partner-input-post-id" name="post_id" value=""/>';
                                    }
                                    ?>
                                    <input type="hidden" name="action" value="st_partner_create_service_flight"/>
                                </form>
                            </div>
                        </div>
                        <?php

...
?>
<div class="st-create">
    <h2><?php esc_html_e('Affiliate Visits','traveler') ?></h2>
</div>
<?php
$per_page = 15;
...
<div class="table-responsive mb30">
    <table>
        <thead>
            <tr>
                <th class="visit-url"><?php esc_html_e('IP','traveler') ?></th>
                <th class="referring-url"><?php esc_html_e('Referring URL','traveler') ?></th>
                <th class="referral-status"><?php esc_html_e('Converted','traveler')?></th>
                <th class="visit-date"><?php esc_html_e('Date','traveler')?></th>
            </tr>
        </thead>
        <tbody>
            <?php
            ...
            foreach ($data_visits as $row)
            {
                ...
                <tr>
                    ...
                    <td><?php echo ($row->referral_id)?esc_html__('Yes','traveler'):esc_html__('No','traveler') ?></td>
                    <td><?php echo date_i18n(get_option('date_format'),strtotime($row->date)) ?></td>
                </tr>
                <?php
            }
            else{
                printf('<tr><td colspan="4" class="text-center">%s</td></tr>',esc_html__('--- Empty ---','traveler'));
            }?>
        </tbody>
    </table>

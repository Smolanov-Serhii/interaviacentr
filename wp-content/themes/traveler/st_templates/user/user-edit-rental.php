 ...
 *
 */
$tour_add_fields = ST_Config::get('partner.add.rental');
...
<div class="st-create-service  ">
    <h2 class="st-header"><?php
        if(!empty($post_id)){
            echo __('Edit Rental', 'traveler');
        }else {
            echo __('Add Rental', 'traveler');
        }
        ?></h2>
                                ...
                                <form method="post" action="" class="st-partner-create-form">
                                    ...
                                    if(!empty($fields)){
                                        foreach ($fields as $kk => $vv){
                                            ...
                                            if(!empty($vv['type'])){?>
                                                <div class="<?php echo esc_attr($class_col); ?> st-partner-field-item <?php echo esc_attr($class_item);?>">
                                                    <?php echo st()->load_template('fields/' . esc_attr($vv['type']), '', array('data' => $vv, 'post_id' => $post_id , 'label' => $vv['label'])); ?>
                                                </div>
                                            <?php
                                            }
                                        }
                                    ...
                                    if(isset($_GET['id'])&& !empty($_GET['id'])){
                                        echo '<input type="hidden" name="btn_update_post_type_rental" value="1"/>';
                                        echo '<input type="hidden" name="post_id" value="'. esc_attr($_GET['id']) .'"/>';
                                    }else{
                                        echo '<input type="hidden" name="btn_insert_post_type_rental" value="1"/>';
                                        echo '<input type="hidden" class="st-partner-input-post-id" name="post_id" value=""/>';
                                    }
                                    ?>
                                    <input type="hidden" name="action" value="st_partner_create_service_rental"/>
                                </form>
                            </div>
                        </div>
                        <?php

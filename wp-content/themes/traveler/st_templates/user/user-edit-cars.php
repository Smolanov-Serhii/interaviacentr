<?php
     ...
     *
     */
$tour_add_fields = ST_Config::get('partner.add.car');
...
<div class="st-create-service">
    <h2 class="st-header"><?php echo __('Add Car', 'traveler'); ?></h2>
                            ...
                            <div class="row">
                                <form method="post" action="" class="st-partner-create-form">
                                    ...
                                    if(isset($_GET['id'])&& !empty($_GET['id'])){
                                        echo '<input type="hidden" name="btn_update_post_type_car" value="1"/>';
                                        echo '<input type="hidden" name="post_id" value="'. esc_attr($_GET['id']) .'"/>';
                                    }else{
                                        echo '<input type="hidden" name="btn_insert_post_type_car" value="1"/>';
                                        echo '<input type="hidden" class="st-partner-input-post-id" name="post_id" value=""/>';
                                    }
                                    ?>
                                    <input type="hidden" name="action" value="st_partner_create_service_car"/>
                                </form>
                            </div>
                        </div>
                        <?php

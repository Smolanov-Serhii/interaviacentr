...
<div class="st-create clearfix">
    ...
    <a class="btn btn-default pull-right" href="<?php echo esc_url( add_query_arg( 'sc' , 'create-rental' , get_permalink() ) ) ?>">
        <?php _e("Add New Rental",'traveler') ?>
    </a>
</div>
<?php
...
<div class="row">
    <div class="col-xs-12">
        <form action="<?php echo esc_url( add_query_arg( 'sc' , 'my-rental' , get_permalink() ) ) ?>" method="get" class="form form-inline partner-search-form">
            <input type="hidden" name="sc" value="my-rental">
            <input class="search-box form form-control" type="text" name="_s" value="<?php echo esc_attr($search) ?>">
            <button class="btn btn-default" type="submit">
                <?php esc_html_e( 'Search Rental', 'traveler' ) ?>
            </button>
        </form>
    </div>
</div>

...
$tour_add_fields = ST_Config::get('partner.add.rental_room');
...
<div class="st-create-service">
    <h2 class="st-header">
        <?php
        if(!empty($post_id)) {
            echo __('Edit Rental Room', 'traveler');
        }else{
            echo __('Add Rental Room', 'traveler');
        }
        ?>
    </h2>
            ...
            <ul class="nav nav-tabs" role="tablist">
                ...
                if(!empty($tour_add_fields['tabs'])){
                    foreach ($tour_add_fields['tabs'] as $k => $v){
                        ...
                        echo '<li role="presentation" class="'. esc_attr($class_active) .'" data-obj="tab"><a href="#'. esc_attr($v['name']) .'" aria-controls="'. esc_attr($v['name']) .'" role="tab" data-toggle="tab">'. $v['label'] . $icon .'</a></li>';
                    }
                }
                ?>
            </ul>
                            ...
                            <div class="row">
                                <form method="post" action="" class="st-partner-create-form">
                                    ...
                                    if(isset($_GET['id'])&& !empty($_GET['id'])){
                                        echo '<input type="hidden" name="btn_update_post_type_rental_room" value="1"/>';
                                        echo '<input type="hidden" name="post_id" value="'. esc_attr($_GET['id']) .'"/>';
                                    }else{
                                        echo '<input type="hidden" name="btn_insert_post_type_rental_room" value="1"/>';
                                        echo '<input type="hidden" class="st-partner-input-post-id" name="post_id" value=""/>';
                                    }
                                    ?>
                                    <input type="hidden" name="action" value="st_partner_create_service_rental_room"/>
                                </form>
                            </div>
                        </div>
                        <?php

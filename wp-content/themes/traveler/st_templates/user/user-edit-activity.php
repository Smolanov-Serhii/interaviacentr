<?php
     ...
     *
     */
$tour_add_fields = ST_Config::get('partner.add.activity');
...
<div class="st-create-service">
    <h2 class="st-header">
        <?php
        if(!empty($post_id)){
            echo __('Edit Activity', 'traveler');
        }else {
            echo __('Add Activity', 'traveler');
        }
        ?>
    </h2>
                            ...
                            <div class="row">
                                <form method="post" action="" class="st-partner-create-form">
                                    ...
                                    if(isset($_GET['id'])&& !empty($_GET['id'])){
                                        echo '<input type="hidden" name="btn_update_post_type_activity" value="1"/>';
                                        echo '<input type="hidden" name="post_id" value="'. esc_attr($_GET['id']) .'"/>';
                                    }else{
                                        echo '<input type="hidden" name="btn_insert_post_type_activity" value="1"/>';
                                        echo '<input type="hidden" class="st-partner-input-post-id" name="post_id" value=""/>';
                                    }
                                    ?>
                                    <input type="hidden" name="action" value="st_partner_create_service_activity"/>
                                </form>
                            </div>
                        </div>
                        <?php

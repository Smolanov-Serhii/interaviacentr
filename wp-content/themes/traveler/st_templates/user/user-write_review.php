...
<div class="st-create" id="write_review_<?php echo esc_attr($item_id); ?>">
    ...
    <form action="" method="post" enctype="multipart/form-data"  id="st_user_comments" class="comment-form">
        <input name="item_id" value="<?php echo esc_attr(STInput::get('item_id'))?>" type="hidden">
        ...
        switch ($review_check) {
          ...
          case "must_login":
                ...
                echo sprintf( st_get_language('you_must').'<a href="'.esc_url($login_link).'">'.__('log in ','traveler').'</a>'.st_get_language('to_write_review'),get_permalink(st()->get_option('user_login_page')));
                echo '</div>';
              break;
          case "need_open":
                echo '<div class="box bg-gray">';
                echo sprintf(__("Review is disabled from administrator" , 'traveler'));
                echo '</div>';
              break;
          case "need_booked":
                echo '<div class="box bg-gray">';
                echo sprintf(st_get_language('you_must') . " ".__("book before write review" , 'traveler'));
                echo '</div>';
              break;
          case "wait_check_out_date":
                echo '<div class="box bg-gray">';
                echo sprintf(st_get_language('you_must') . " ".__("experience of this tour to write review" , 'traveler'));
                echo '</div>';
              break;
          case "reviewed":
                echo '<div class="box bg-gray">';
                echo st_get_language('you_have_been_post_a_review').' '. balanceTags($name);
                echo '</div>';
              break;
      }

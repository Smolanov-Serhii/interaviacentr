...
<table class="table table-bordered table-striped table-booking-history">
    ...
    <?php
        ...
        while( have_posts() ): the_post();
            ...
            $action_cancel = '';
            if( (float)$post->cancel_refund > 0 && $post->cancel_refund_status == 'pending' ){
                $action_cancel = '<a data-toggle="modal" data-target="#with-refund-modal" href="javascript: void(0);" class="with_a_refund btn btn-primary btn-xs" data-order_id="'. esc_attr($post->order_item_id) .'" data-order_encrypt="'. TravelHelper::st_encrypt( $post->order_item_id ) .'">'. __('with refund', 'traveler') .'</a>';
            }
...
?>
    <tr>
        ...
        <td><a href="<?php echo esc_url($class_user->get_link_order_item( $post->st_booking_id )); ?>"><?php echo esc_html($class_user->get_title_order_item( $post->st_booking_id )) ?></a></td>
        ...
        <td class="hidden-xs"><?php echo balanceTags($status_string . $action_cancel); ?></td>
    </tr>
    <?php endwhile; ?>
...
<?php
    ...
    echo '<h5>'. __('The refund list is empty', 'traveler').'</h5>';
endif;
?>
    ...
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button id="" type="button" class="next btn btn-primary hidden"><?php echo __('Complete this Refund', 'traveler'); ?></button>
                <button type="button" class="btn btn-default" data-dismiss="modal"><?php echo esc_html__('Close','traveler');?></button>
            </div>
        </div>
    </div>
</div>

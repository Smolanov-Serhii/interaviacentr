        ...
        <li>
            <ul class="booking-item-payment-price">
	            <?php if($tour_price_by != 'fixed_depart'){ ?>
                    <li>
                        <p class="booking-item-payment-price-title">
                            <?php echo __('Type Tour', 'traveler'); ?>
                        </p>
                    ...
                    <?php if($type_tour == 'daily_tour'): ?>
                        <li>
                            ...
                            <p class="booking-item-payment-price-amount">
                                <?php echo esc_html(date_i18n(TravelHelper::getDateFormat(), strtotime($check_in)) . ($starttime == '' ? '' : ' - ' . $starttime)); ?>
                            </p>
                        </li>
                    <?php else: ?>
                        <li>
                            ...
                            <p class="booking-item-payment-price-amount">
                                <?php echo esc_html(date_i18n(TravelHelper::getDateFormat(), strtotime($check_in))); ?> -
                                <?php echo esc_html(date_i18n(TravelHelper::getDateFormat(), strtotime($check_out)) . ($starttime == '' ? '' : ' - ' . $starttime)); ?>
                            </p>
                        </li>
                    <?php endif; ?>
                    <?php if($type_tour == 'daily_tour' and $duration): ?>
                        <li>
                            <p class="booking-item-payment-price-title"><?php  _e('Duration','traveler')?> </p>
                            <p class="booking-item-payment-price-amount">
                                <?php
                ...
                <?php }else{ ?>
                    ...
                    <li>
                        <p class="booking-item-payment-price-title"><?php  _e('Start','traveler')?> </p>
                        <p class="booking-item-payment-price-amount">
				            <?php
                    ...
                    <li>
                        <p class="booking-item-payment-price-title"><?php  _e('End','traveler')?> </p>
                        <p class="booking-item-payment-price-amount">
				            <?php
                ...
                <?php if($adult_number){?>
                    <li>
                        <p class="booking-item-payment-price-title"><?php _e('Number of Adult','traveler') ?> </p>


                        <p class="booking-item-payment-price-amount"><?php echo esc_html($adult_number); ?>
                        </p>
                ...
                <?php if($child_number){?>
                    <li>
                        <p class="booking-item-payment-price-title"><?php _e('Number of Children','traveler') ?> </p>
                        <p class="booking-item-payment-price-amount"><?php echo esc_html($child_number); ?>
                        </p>
                    </li>
                <?php } ?>
                <?php if($infant_number){?>
                    <li>
                        <p class="booking-item-payment-price-title"><?php _e('Number of Infant','traveler') ?> </p>
                        <p class="booking-item-payment-price-amount"><?php echo esc_html($infant_number); ?>
                        </p>
                    </li>
                <?php } ?>
                <?php
                if(isset($item['data']['deposit_money'])):
                    ...
                    if(floatval($deposit['amount']) > 0) :
	                    $deposite_text = __('Deposit percent', 'traveler');
                        if($deposit['type'] == 'amount'){
	                        $deposite_text = __('Deposit amount', 'traveler');
                        }
                        ?>
                        <li>
                            <p class="booking-item-payment-price-title"><?php echo esc_html($deposite_text); ?> </p>
                ...
                <?php if(isset($extras['value']) && is_array($extras['value']) && count($extras['value'])): ?>
                    <li>
                        <p class="booking-item-payment-price-title"><?php _e("Extra",'traveler') ?></p><br />
                        <p class="booking-item-payment-price-amount">
                            <?php
                ...
                <?php if(is_array($hotel_package) && count($hotel_package)): ?>
                    <li>
                        <p class="booking-item-payment-price-title"><?php _e("Selected Hotel Package",'traveler') ?></p><br />
                        <p class="booking-item-payment-price-amount">
                            <?php
                ...
                <?php if(is_array($activity_package) && count($activity_package)): ?>
                    <li>
                        <p class="booking-item-payment-price-title"><?php _e("Selected Activity Package",'traveler') ?></p><br />
                        <p class="booking-item-payment-price-amount">
                            <?php
                ...
                <?php if(is_array($car_package) && count($car_package)): ?>
                    <li>
                        <p class="booking-item-payment-price-title"><?php _e("Selected Car Package",'traveler') ?></p><br />
                        <p class="booking-item-payment-price-amount">
                            <?php
	            ...
	            <?php if(is_array($flight_package) && count($flight_package)): ?>
                    <li>
                        <p class="booking-item-payment-price-title"><?php _e("Selected Flight Package",'traveler') ?></p><br />
                        <p class="booking-item-payment-price-amount">
				            <?php
...
<div class="booking-item-coupon p10">
    <form method="post" action="<?php the_permalink() ?>">
        ...
        <div class="form-group">
            <label for="field-coupon_code"><?php _e('Coupon Code', 'traveler') ?></label>
        ...
        <?php if(st()->get_option('use_woocommerce_for_booking','off') == 'off' && st()->get_option('booking_modal','off') == 'on' ){ ?>
            ...
            <button class="btn btn-primary add-coupon-ajax"><?php _e('Apply Coupon', 'traveler') ?></button>
            <div class="alert alert-danger hidden">
            </div>
        <?php }else{ ?>
            <button class="btn btn-primary" type="submit"><?php _e('Apply Coupon', 'traveler') ?></button>
        <?php } ?>
    </form>
</div>
<div class="booking-item-payment-total text-right">
    ...
    <table border="0" class="table_checkout">
        ...
        <tr>
            <td class="text-left title"><?php echo __('Adult Price', 'traveler'); ?></td>
        ...
        <tr>
            <td class="text-left title"><?php echo __('Children Price', 'traveler'); ?></td>
        ...
        <tr>
            <td class="text-left title"><?php echo __('Infant Price', 'traveler'); ?></td>
        ...
        <?php }else{ ?>
            <tr>
                <td class="text-left title"><?php echo __('Price', 'traveler'); ?></td>
        ...
        <tr>
            <td class="text-left title"><?php echo __('Subtotal', 'traveler'); ?></td>
        ...
        if(isset($item['data']['deposit_money']) && count($item['data']['deposit_money']) && floatval($item['data']['deposit_money']['amount']) > 0):
            ...
            ?>
            <tr>
                <td class="text-left title"><?php echo __('Total', 'traveler'); ?></td>
            ...
            <tr style="border-top: 1px solid #CCC; font-size: 20px; text-transform: uppercase; margin-top: 20px;">
                <td class="text-left title"><?php echo __('Deposit', 'traveler'); ?></td>
                <td class="text-right ">
            ...
            ?>
                <tr>
                    <td class="text-left title"><?php echo __('Fee', 'traveler'); ?></td>
                    <td class="text-right "><strong><?php echo TravelHelper::format_money($item['data']['booking_fee_price']); ?></strong></td>
                </tr>
            <?php } ?>
            <tr>
                <td class="text-left title " style="border: none; text-transform: uppercase;"><strong><?php echo __('Pay Amount', 'traveler'); ?></strong></td>
        ...
        <?php else: ?>
            ...
            <?php } ?>
            <tr style="border-top: 1px solid #CCC; font-size: 20px; text-transform: uppercase; margin-top: 20px;">
                <td class="text-left title"><?php echo __('Pay Amount', 'traveler'); ?></td>
                <td class="text-right "><strong><?php echo TravelHelper::format_money($price_with_tax); ?></strong></td>
            </tr>
        <?php endif; ?>
    </table>

...
if (!empty($ids)) {
    ...
    <table class="wp-list-table widefat fixed striped stour-list-hotel" data-type="hotel">
        ...
        <?php foreach ($ids as $k => $v) { ?>
            <tr>
                ...
                <td data-name="<?php echo __('Hotel name', 'traveler'); ?>">
                    ...
                    echo balanceTags(TravelHelper::rate_to_string($star));
                    echo '</ul>';
                    ?>
                </td>
                <td data-name="<?php echo __('Price', 'traveler'); ?>"><input type="text" class="hotel-price" value="<?php echo (!empty($arr_tour_package) && isset($arr_tour_package[$v['ID']])) ? $arr_tour_package[$v['ID']] : ''; ?>"/></td>
            </tr>
        <?php } ?>
        </tbody>
    </table>
    ...
    echo __('No hotels found!', 'traveler');
}
?>
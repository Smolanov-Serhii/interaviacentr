            ...
            if (!empty($terms)) {
                ...
                <ul class="booking-item-features booking-item-features-expand mb30 clearfix">
                    ...
                    foreach ($terms as $key2 => $value2) {
                        ?>
                        <li class="<?php if($item_col) echo 'col-sm-'.esc_attr($item_col);?>">
                            <?php if (function_exists('get_tax_meta') and $icon = get_tax_meta($value2->term_id, 'st_icon')): ?>
                                <i class="<?php echo esc_attr(TravelHelper::handle_icon($icon)) ?>"></i>
                            <?php endif; ?>

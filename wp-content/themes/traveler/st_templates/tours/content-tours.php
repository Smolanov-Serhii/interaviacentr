...
<div class="row">
    <div class="col-md-12">
                    ...
                    <ul class="nav nav-pills">
                        ...
                        if (!empty($allOrderby) and is_array($allOrderby)):
                            foreach ($allOrderby as $key => $value) {
                                ...
                                if ($active == $key) {
                                    echo '<li class="active"><a href="' . esc_url($link) . '">' . balanceTags($value['name']) . '</a>';
                                } elseif ($key == 'new' and empty($active)) {
                                    echo '<li class="active"><a href="' . esc_url($link) . '">' . balanceTags($value['name']) . '</a>';
                                } else {
                                    echo '<li><a href="' . esc_url($link) . '">' . balanceTags($value['name']) . '</a>';
                                }
                            }
                        endif;
                        ?>
        ...
        if ($st_style == '1') {
            echo '<ul class="booking-list loop-tours style_list">' . balanceTags($content) . '</ul>';
        }
        if ($st_style == '2') {
            wp_enqueue_script('jquery.matchHeight-min');

            echo '<div class="row row-wrap">' . balanceTags($content) . '</div>';
        }
        ?>

...
<?php if(empty($hidde_button)){ ?><div><form method="get" action=""><?php }else { ?><div class=" form-group form-group-<?php echo esc_attr($field_size) ?> "><?php }?>
    ...
    if ($data['title']){
        echo  "<label>".balanceTags($data['title'])."</label>";
        }
    }
    ...
    echo '<input name="price_range" type="text" value="'.esc_html($value_show).'" class="price-slider" data-symbol="'.TravelHelper::get_current_currency('symbol').'" data-min="'.esc_attr($min).'" data-max="'.esc_attr($max).'" data-step="'.st()->get_option('search_price_range_step',0).'">';
